                                        ...
                                        <div key={index} className="relative flex flex-col md:flex-row items-center gap-12 p-8 md:p-10 rounded-[2rem] border border-gray-200 bg-white shadow-sm hover:shadow-xl transition-all duration-500 group overflow-hidden">
                                            ...
                                            <div className="flex-1 space-y-6">
                                                ...
                                                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                                    {(isVO || (['cv', 'rups cv (perubahan)', 'pt umum', 'cv to pt umum', 'upgrade ke pt umum', 'rups pt (pengurus/oper saham)', 'rups pt (kbli/kedudukan/nama pt/modal)', 'pt pma (pt asing)', 'pt pma + nib dan pkkpr', 'rups pt pma (pt asing)', 'perubahan yayasan', 'koperasi', 'ud (usaha dagang)', 'penutupan pt perorangan', 'akta cabang pt', 'akta cabang cv', 'nib pt pma', 'pengumuman koran', 'nib badan (perusahaan)', 'nib pribadi', 'iumk (izin usaha mikro kecil)', 'tdp (tanda daftar perusahaan)', 'siup (surat izin usaha perdagangan)', 'situ (surat izin tempat usaha)', 'ho (izin gangguan)', 'izin usaha toko retail'].includes(service?.name?.toLowerCase() || '') && (header.key === 'business' || header.label.toLowerCase().includes('business')))) ? (
                                                        <>
                                                            {[
